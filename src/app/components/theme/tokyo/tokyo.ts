...
export class Tokyo {
    ...
    forkJoin([home_page$, categories$]).subscribe({
      complete: () => {
        this.store.select(ThemeState.homePage).subscribe({
          next: (homePage: { id: number; slug: string; content: ActiveThemeContent } | null) => {
          ...
          main_content: {
            sidebar: {
              ...
              right_side_banners: {
                ...
                banner_2: {
                  image_url:
                    homePage?.content?.main_content?.sidebar?.right_side_banners?.banner_2
                      ?.image_url,
                  redirect_link: {
                    link: homePage?.content?.main_content?.sidebar?.right_side_banners?.banner_2
                      ?.redirect_link?.link,
                    link_type:
                      homePage?.content?.main_content?.sidebar?.right_side_banners?.banner_2
                        ?.redirect_link?.link_type,
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: ITokyo): number[] {
    const result: number[] = [];
