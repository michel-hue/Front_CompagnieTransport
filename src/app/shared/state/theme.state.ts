...
export class ThemesStateModel<T = unknown> {
  themes = {
    data: [] as IThemes[],
  };
  homePage: { id: number; slug: string; content: T } | null = null;
}
export type ActiveThemeContent =
  | IContent
  | IContentTokyo
  | IContentRome
  | IContentOsaka
...
@State<ThemesStateModel<ActiveThemeContent>>({
  name: 'theme',
  defaults: {
    themes: {
      data: [],
...
export class ThemeState {
  ...
  @Selector()
  static homePage<T>(state: ThemesStateModel<T>) {
